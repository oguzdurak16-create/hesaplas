...
export default function ToolPage({ params }) {
  ...
  const defaultFaq = [
    { q: `${tool.title} nasıl kullanılır?`, a: `Formdaki ${tool.fields.length ? inputLabels.map((label) => label.toLocaleLowerCase('tr-TR')).slice(0, 3).join(', ') : 'gerekli'} bilgilerini girin. Sonuç tarayıcınızda anında hesaplanır.` },
    ...
    <div className="container tool-page-container">
      ...
      <header className={`tool-page-hero category-${tool.category}`}>
        ...
        <div className="tool-hero-copy">
          <span className="eyebrow">{category?.name} {tool.badge && `• ${tool.badge}`}</span>
          <h1>{tool.title}</h1>
          <p>{tool.description}</p>
          <div className="tool-hero-meta"><small className="updated-label">Güncellendi: {updatedLabel}</small><FavoriteButton slug={tool.slug} /></div>
          <div className="v7-trust-row" aria-label="Araç özellikleri"><span><i/> Ücretsiz</span><span><i/> Üyelik yok</span><span><i/> Tarayıcıda hesaplanır</span></div>
        </div>
      </header>
      ...
      <section className="v7-content-intro" aria-label="Hesaplama özeti">
        <article className="v7-guide-card">
          ...
          <h2>{tool.shortTitle || tool.title} nasıl hesaplanır?</h2>
          <p>{guide.intro}</p>
          <div className="v7-formula"><span>Kullanılan yaklaşım</span><p>{formulaText}</p></div>
        </article>
        <aside className="v7-facts-card">
          <span className="eyebrow">Kontrol listesi</span>
          <h2>Sonucu doğru okuyun</h2>
          <div className="v7-fact-list">
            <div><b>1</b><span>Girdiğiniz değerlerin birim ve dönemlerinin aynı olduğundan emin olun.</span></div>
            <div><b>2</b><span>{guide.evaluate}</span></div>
            <div><b>3</b><span>Gerekirse farklı değerlerle senaryo kaydedip sonuçları karşılaştırın.</span></div>
          </div>
        </aside>
      </section>
      ...
      <section className="content-layout">
        <article className="seo-article">
          <span className="eyebrow">Detaylı açıklama</span>
          <h2>{tool.title} nedir?</h2>
          <p>{guide.intro}</p>
          ...
          <p>Paylaş düğmesi girdiğiniz değerleri sayfa adresine ekler. Kaydet seçeneğiyle en fazla üç sonucu yalnızca kendi tarayıcınızda tutup karşılaştırabilirsiniz.</p>
          {!!tool.sources?.length && <div className="source-box"><strong>Resmî ve birincil kaynaklar</strong>{tool.sources.map((source) => <a key={source.url} href={source.url} target="_blank" rel="noreferrer noopener">{source.label} ↗</a>)}</div>}
        </article>
        <aside className="faq-card">
          <span className="eyebrow">Sık sorulanlar</span>
          <h2>Merak edilenler</h2>
          {faq.map((row) => <details key={row.q}><summary>{row.q}<span>+</span></summary><p>{row.a}</p></details>)}
        </aside>
      </section>
